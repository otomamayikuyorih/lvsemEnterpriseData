# Demo 5: グリーンホテルの生成AI、社員幸福、環境パフォーマンス

## データ

- Dataset ID: `green_hotel_gai_employee_happiness`
- Source: https://data.mendeley.com/datasets/999m5mhdkx
- License: CC BY 4.0 (https://creativecommons.org/licenses/by/4.0/)
- 分析に使った有効行数: 219
- ブートストラップ回数: 300

## 研究背景

生成AIは、ホテル業務の効率化、情報共有、管理プロセス改善に使われ始めています。 一方で、AI導入が従業員幸福やサステナビリティ成果にどう関係するかは、 単なる技術導入以上の組織マネジメント課題です。 このデモでは、生成AI活用、管理プロセス、社員幸福、持続可能性、環境成果の関係を探索します。

## モデル

`AIG` を生成AI活用、`EHA` を社員幸福、`SUS`/`EPG` を持続可能性・環境成果として扱う探索モデルです。

### 測定ブロック

- `AIG`: `AIG1`, `AIG2`, `AIG3`, `AIG4`
- `MPG`: `MPG1`, `MPG2`, `MPG3`, `MPG4`
- `EHA`: `EHA1`, `EHA2`, `EHA3`, `EHA4`, `EHA5`
- `SUS`: `SUS1`, `SUS2`, `SUS3`, `SUS4`, `SUS5`
- `EPG`: `EPG1`, `EPG2`, `EPG3`, `EPG4`, `EPG5`

### 構造パス

- `AIG` -> `MPG`
- `AIG` -> `EHA`
- `MPG` -> `SUS`
- `EHA` -> `SUS`
- `SUS` -> `EPG`

### パス図

![Demo 5: グリーンホテルの生成AI、社員幸福、環境パフォーマンス path diagram](figures/green_hotel_gai_employee_happiness_path.png)

## 信頼性・妥当性の要約

```text
 block alpha composite_reliability   ave
   AIG 0.786                 0.863 0.613
   MPG 0.861                 0.906 0.706
   EHA 0.854                 0.896 0.634
   SUS 0.804                 0.867 0.571
   EPG 0.828                 0.877 0.588
```

### ローディング要約

```text
 block min_loading mean_loading max_loading items
   AIG       0.662        0.780       0.834     4
   EHA       0.697        0.795       0.851     5
   EPG       0.696        0.765       0.847     5
   MPG       0.775        0.839       0.883     4
   SUS       0.609        0.748       0.850     5
```

## 構造モデル

### パス係数

```text
       path  beta
 AIG_to_MPG 0.725
 AIG_to_EHA 0.828
 MPG_to_SUS 0.593
 EHA_to_SUS 0.288
 SUS_to_EPG 0.922
```

### ブートストラップ

```text
       path  beta boot_se t_value p_value_approx
 AIG_to_MPG 0.725   0.035  20.709          0.000
 AIG_to_EHA 0.828   0.027  30.658          0.000
 MPG_to_SUS 0.593   0.079   7.480          0.000
 EHA_to_SUS 0.288   0.087   3.319          0.001
 SUS_to_EPG 0.922   0.007 125.129          0.000
```

### R2

```text
 construct r_squared
       MPG     0.526
       EHA     0.685
       SUS     0.727
       EPG     0.850
```

## 結果の短い読み取り

- いちばん強い関係は、持続可能性実践が高いほど環境パフォーマンスも高い傾向でした (β=0.922)。
- はっきりした関係として読めるものは、持続可能性実践が高いほど環境パフォーマンスも高い傾向 (β=0.922)、生成AI活用が高いほど社員幸福も高い傾向 (β=0.828)、生成AI活用が高いほど管理プロセスも高い傾向 (β=0.725)、管理プロセスが高いほど持続可能性実践も高い傾向 (β=0.593)、社員幸福が高いほど持続可能性実践も高い傾向 (β=0.288)です。
- 一方で、今回のデータでは明確とは言いにくい関係は、なしです。
- モデルが最もよく説明できているのは環境パフォーマンスで、ばらつきの約85%をこのモデルで説明しています (R2=0.850)。
- 各構成概念の質問項目はおおむね同じ概念を測れており、測定面では大きな問題は見えません。

## 簡単な考察

この結果は、「生成AIを使っているホテルほど環境成果がよい」と単純に読むより、 もう一段ていねいに見る必要があります。 生成AIの活用は、まず管理プロセスの整備や社員幸福と強く関係しています。 たとえば、情報共有、業務の見える化、意思決定の支援、従業員の負担軽減などが進むと、 社員が働きやすくなり、サステナビリティの取り組みも回りやすくなる、というイメージです。 そのうえで、持続可能性の実践がしっかりしているホテルほど、環境パフォーマンスも高い傾向があります。 つまり、AIは魔法の道具として直接成果を出すというより、 管理の仕組みや社員の働きやすさを支え、それが環境への取り組みを強めることで成果につながる、と考えると分かりやすいです。 実務的には、AIツールを入れるだけでなく、業務プロセス、社員の納得感、サステナビリティ施策をセットで設計することが重要です。

## メモ

- このデモは `lvsem` の軽量ワークフローに合わせ、測定項目から潜在変数スコアを作成し、構造パスを標準化回帰として推定しています。
- 欠損や非数値は、指定した測定項目を数値化したうえで完全ケースのみを使いました。
- 研究論文の厳密な再現ではなく、`lvsemEnterpriseData` に収録した企業・組織内データの利用例です。

