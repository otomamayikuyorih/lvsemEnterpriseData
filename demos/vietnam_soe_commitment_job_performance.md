...
結果では、規範的コミットメントと継続的コミットメントがOCBに強く関係し、 OCBも職務パフォーマンスと明確に結びついています。 一方で、情緒的コミットメントからOCBへの直接パスは弱く、 このデータでは「組織に残る理由」や「組織に対する義務感」のような側面が、 自発的な協力行動を説明しやすいことが示唆されます。 ただし、単一の探索モデルなので、因果解釈よりも組織内関係の把握として読むのが妥当です。
